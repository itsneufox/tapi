    ...
    .action(async (options) => {
      try {
        ...
        if (!fs.existsSync(manifestPath)) {
          logger.error('No pawn.json manifest found. Run "npt init" first or create a manifest file.');
          process.exit(1);
        }
        ...
        const compilerOptions = manifest.compiler?.options || ['-;+', '-(+', '-\\+', '-Z+'];
        ...
        compiler.stdout.on('data', (data) => {
          ...
          for (const line of lines) {
            ...
            if (match) {
              ...
              console.log(formatProblem(file, parseInt(lineNum), severity, code, message));
            } else {
        ...
        compiler.on('close', (code) => {
          if (code === 0) {
            ...
            const successMatch = output.match(/Code\s*:\s*(\d+)\s*bytes\nData\s*:\s*(\d+)\s*bytes\nStack\/Heap\s*:\s*(\d+)\s*bytes\nEstimated usage\s*:\s*(\d+)\s*cells\nTotal requirements\s*:\s*(\d+)\s*bytes/);
            
            if (successMatch) {
              logger.routine('Compilation statistics:');
              logger.info(`${path.basename(inputFile || '')} compiled successfully (${successMatch[5]} bytes)`);
