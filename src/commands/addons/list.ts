        ...
        if (options.all) {
          ...
          if (installedAddons.length > 0) {
            ...
            for (const addon of installedAddons) {
              ...
              logger.info(
                `  ${status} ${addon.name}${version} - ${addon.description}`
              );
            }
            logger.info('');
          }
          ...
          try {
            ...
            if (availableAddons.length > 0) {
              for (const addon of availableAddons) {
                logger.info(
                  `  - ${addon.name} - ${addon.description || 'No description available'}`
                );
              }
            } else {
            ...
            logger.warn(
              `Could not load available addons: ${error instanceof Error ? error.message : 'unknown error'}`
            );
            logger.info(
              '  Run "tapi addon install <addon>" to install specific addons'
            );
          }
        } else {
          ...
          if (hasConflicts) {
            ...
            for (const [commandName, conflictingAddons] of conflicts) {
              logger.warn(
                `  ${commandName}: ${conflictingAddons.length} addons trying to override`
              );
              for (const addon of conflictingAddons) {
                logger.warn(
                  `    - ${addon.name} (priority: ${addon.priority || 0})`
                );
              }
            }
          }
          ...
          if (stats.totalAddonCommands > 0) {
            ...
            logger.info(
              `  Overridden commands: ${stats.overriddenCommands.length}`
            );
