...
export class AddonLoader {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && this.isRetryableError(lastError)) {
          logger.warn(
            `Addon load attempt ${attempt} failed, retrying... (${lastError.message})`
          );
   ...
   */
  private provideRecoverySuggestions(
    addonPath: string,
    error: Error | null
  ): void {
    ...
    if (error?.message.includes('Addon not found')) {
      ...
      logger.info(
        '  - Ensure the addon directory exists and contains valid files'
      );
    } else if (error?.message.includes('Invalid addon structure')) {
      logger.info(
        '  - Ensure your addon exports a class that extends TapiAddon'
      );
      logger.info(
        '  - Check that your addon has proper constructor and methods'
      );
    ...
    logger.info(
      `  - Try reinstalling the addon: 'tapi addon uninstall <name> && tapi addon install <name>'`
    );
  }
  ...
  async unloadAddon(name: string): Promise<void> {
    ...
    try {
      ...
      logger.error(
        `Failed to unload addon ${name}: ${error instanceof Error ? error.message : 'unknown error'}`
      );
      throw error;
    }
  }
      ...
      for (const addonDir of addonDirs) {
        ...
        if (fs.existsSync(packageJsonPath)) {
          try {
            ...
            logger.warn(
              `Failed to load addon from ${addonDir}: ${error instanceof Error ? error.message : 'unknown error'}`
            );
          }
        }
      }
  ...
  private validateAddon(addon: unknown): asserts addon is TapiAddon {
    ...
    for (const [hookName, hook] of Object.entries(
      addonObj.hooks as Record<string, unknown>
    )) {
