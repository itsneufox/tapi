    ...
    .action(async (options) => {
      ...
      try {
        ...
        if (options.all) {
          ...
          logger.info(
            `Found ${failedAddons.length} addon(s) with errors: ${failedAddons.join(', ')}`
          );
          ...
          for (const addonName of failedAddons) {
            try {
              ...
              stillFailed++;
              logger.warn(
                `Still failing: ${addonName} (${error instanceof Error ? error.message : 'unknown error'})`
              );
            }
          }
          ...
          if (stillFailed > 0) {
            ...
            logger.info(
              '  - Try reinstalling: tapi addon uninstall <name> && tapi addon install <name>'
            );
          ...
          logger.info(
            '  - Disable problematic addons: tapi addon disable <name>'
          );
          logger.info(
            '  - Reinstall addons: tapi addon uninstall <name> && tapi addon install <name>'
          );
        }
      } catch (error) {
        logger.error(
