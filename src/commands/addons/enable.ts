    ...
    .action(async (addonName: string) => {
      ...
      try {
        ...
        if (!addon || !addon.installed) {
          ...
          logger.info(
            `Run 'tapi addon install ${addonName}' to install it first`
          );
          return;
        }
        ...
        logger.info('');
        logger.info(
          'The addon is now active and will run its hooks during project operations.'
        );
      } catch (error) {
        logger.error(
          `Failed to enable addon: ${error instanceof Error ? error.message : 'unknown error'}`
        );
        process.exit(1);
      }
    });
