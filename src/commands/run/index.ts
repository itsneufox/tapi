  ...
  program
    ...
    .action(async (scriptName, options) => {
      ...
      try {
        ...
        if (!scriptName) {
          logger.error(
            'Please specify a script name or use --list to see available scripts'
          );
...
async function runScript(
  ...
  try {
    ...
    logger.error(
      `Script "${scriptName}" failed: ${error instanceof Error ? error.message : 'unknown error'}`
    );
    process.exit(1);
  }
}
