...
export const SERVER_STATE_FILE = path.join(os.homedir(), '.npt', 'server_state.json');
...
export function saveServerState(state: ServerState): void {
  try {
    ...
    logger.error(`Failed to save server state: ${error instanceof Error ? error.message : 'unknown error'}`);
  }
}
...
export function loadServerState(): ServerState {
  try {
    ...
    logger.warn(`Failed to load server state: ${error instanceof Error ? error.message : 'unknown error'}`);
  }
  return {};
}
...
export function clearServerState(): void {
  try {
    ...
    logger.warn(`Failed to clear server state: ${error instanceof Error ? error.message : 'unknown error'}`);
  }
}
...
export function isServerRunning(): boolean {
  ...
  try {
    if (process.platform === 'win32') {
      const result = require('child_process').spawnSync('tasklist', ['/FI', `PID eq ${state.pid}`], {encoding: 'utf8'});
