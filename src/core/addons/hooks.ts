...
export class HookManager {
   ...
   * Provide recovery suggestions for hook failures
   */
  private provideHookRecoverySuggestions(
    event: string,
    errorMsg: string
  ): void {
    ...
    if (
      ...
      logger.info(
        '  - Check that the hook context contains expected properties'
      );
      logger.info(
        '  - Verify hook implementation matches the expected interface'
      );
    } else if (
    ...
    logger.info(
      `  - Disable the problematic addon: 'tapi addon disable <addon-name>'`
    );
