...
async function handleUpdate(options: {
  ...
  try {
    ...
    if (latestRelease.body) {
      logger.info('Changes:');
      logger.info(
        latestRelease.body.substring(0, 500) +
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode === 404) {
            reject(
              new Error(
                'No releases found. This version of pawnctl may be pre-release or the repository has no published releases yet.'
              )
            );
            return;
          }
          ...
          if (validReleases.length === 0) {
            const hasPreReleases = releases.some(
              (r) => !r.draft && r.prerelease
            );
            if (hasPreReleases) {
              reject(
                new Error(
                  'No stable releases found. Use "pawnctl update --pre" to include pre-release versions.'
                )
              );
            } else {
