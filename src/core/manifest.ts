...
export async function generatePackageManifest(options: any): Promise<void> {
  try {
    ...
    logger.error(`Failed to create manifest file: ${error instanceof Error ? error.message : 'unknown error'}`);
...
export async function loadManifest(): Promise<PackageManifest | null> {
  try {
    ...
    logger.error(`Failed to read manifest file: ${error instanceof Error ? error.message : 'unknown error'}`);
    return null;
  }
}
 ...
 */
export async function updateManifest(updates: Partial<PackageManifest>): Promise<boolean> {
  try {
    ...
    logger.error(`Failed to update manifest file: ${error instanceof Error ? error.message : 'unknown error'}`);
    return false;
  }
}
