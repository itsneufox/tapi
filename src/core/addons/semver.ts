...
export class SemVer {
   ...
   */
  private static satisfiesSimple(
    version: string,
  ...
  private static satisfiesCaret(
    ...
    return (
      versionParts.major === target.major &&
  ...
  private static satisfiesRange(version: string, range: string): boolean {
    ...
    for (let i = 0; i < parts.length; i += 2) {
      ...
      if (minPart) {
        ...
        range.includeMin =
          minConstraint.operator === '>=' || minConstraint.operator === '=';
      }
      ...
      if (maxPart) {
        ...
        range.includeMax =
          maxConstraint.operator === '<=' || maxConstraint.operator === '=';
      }
    ...
    return ranges.some((range) => {
      ...
      if (range.min) {
        ...
        satisfies =
          satisfies &&
          (range.includeMin ? minComparison >= 0 : minComparison > 0);
      }
      ...
      if (range.max) {
        ...
        satisfies =
          satisfies &&
          (range.includeMax ? maxComparison <= 0 : maxComparison < 0);
      }
  ...
  static detectConflicts(
    constraints: string[]
  ): Array<{ constraint1: string; constraint2: string; reason: string }> {
   ...
   */
  private static constraintsCompatible(
    constraint1: string,
    constraint2: string
  ): boolean {
